...
export const renderMonitorList = (
  ...
  const renderItem = (monitor, idx) => (
    <div key={idx} className='p-2 hover:bg-[color-mix(in_srgb,var(--glass-bg)_90%,transparent)] rounded-lg transition-colors'>
  ...
  return Object.entries(grouped).map(([gname, list]) => (
    <div key={gname || 'default'} className='mb-2'>
      {gname && (
        <>
          <div className='text-md font-semibold text-[var(--semi-color-text-2)] px-2 py-1'>
            {gname}
          </div>
          <Divider />
        </>
