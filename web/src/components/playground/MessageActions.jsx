      ...
      {message.content && (
        <Tooltip content={t('复制')} position='top'>
          <Button
            ...
            className={`!rounded-full !text-[var(--semi-color-text-2)] hover:!text-green-600 hover:!bg-[color-mix(in_srgb,var(--glass-bg)_90%,transparent)] ${styleState.isMobile ? '!w-6 !h-6' : '!w-7 !h-7'} !p-0 transition-all`}
            aria-label={t('复制')}
          />
        </Tooltip>
      )}
      ...
      {canEdit && (
        ...
        >
          <Button
            ...
            disabled={shouldDisableActions}
            className={`!rounded-full ${shouldDisableActions ? '!text-[var(--semi-color-text-2)] !opacity-50 !cursor-not-allowed' : '!text-[var(--semi-color-text-2)] hover:!text-yellow-600 hover:!bg-[color-mix(in_srgb,var(--glass-bg)_90%,transparent)]'} ${styleState.isMobile ? '!w-6 !h-6' : '!w-7 !h-7'} !p-0 transition-all`}
            aria-label={t('编辑')}
          />
        </Tooltip>
      )}
      ...
      {canToggleRole && !isLoading && (
        ...
        >
          <Button
            ...
            disabled={shouldDisableActions}
            className={`!rounded-full ${shouldDisableActions ? '!text-[var(--semi-color-text-2)] !opacity-50 !cursor-not-allowed' : message.role === 'system' ? '!text-purple-500 hover:!text-purple-700 hover:!bg-[color-mix(in_srgb,var(--glass-bg)_90%,transparent)]' : '!text-[var(--semi-color-text-2)] hover:!text-purple-600 hover:!bg-[color-mix(in_srgb,var(--glass-bg)_90%,transparent)]'} ${styleState.isMobile ? '!w-6 !h-6' : '!w-7 !h-7'} !p-0 transition-all`}
            aria-label={
              message.role === 'assistant'
                ? t('切换为System角色')
                : t('切换为Assistant角色')
      ...
      {!isLoading && (
        ...
        >
          <Button
            ...
            disabled={shouldDisableActions}
            className={`!rounded-full ${shouldDisableActions ? '!text-[var(--semi-color-text-2)] !opacity-50 !cursor-not-allowed' : '!text-[var(--semi-color-text-2)] hover:!text-red-600 hover:!bg-[color-mix(in_srgb,var(--glass-bg)_90%,transparent)]'} ${styleState.isMobile ? '!w-6 !h-6' : '!w-7 !h-7'} !p-0 transition-all`}
            aria-label={t('删除')}
          />
        </Tooltip>
      )}
